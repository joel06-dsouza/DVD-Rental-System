...
ngAfterViewInit() {
  this.StoreID = localStorage.getItem('StoreId');
  this.loadData(); // Make sure loadData() is called
  this.dataSource.paginator = this.paginator;
}
...
loadData() {
  this.dvdRentalService.getPaginatedData().subscribe(
    (response: ApiResponse<FilmInfoPage>) => {
      ...
      if (response.data && response.data.filmInfoPage && response.data.filmInfoPage.content) {
        this.dataSource.data = response.data.filmInfoPage.content;
        this.dataSource.paginator = this.paginator;
      }
